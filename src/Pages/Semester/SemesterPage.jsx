...
export default function SemesterPage() {
  ...
  const handleSave = async (payload) => {
    try {
      if (editingItem) {
        await api.post("/semester", payload); // Using POST as per user's example for add/update
        Swal.fire({
          icon: "success",
          title: "Semester details updated successfully!",
          toast: true,
          position: "top-end",
          timer: 3000,
        ...
        Swal.fire({
          icon: "success",
          title: "New semester added successfully!",
          toast: true,
          position: "top-end",
          timer: 3000,
          showConfirmButton: false,
      ...
      console.error("Failed to save semester information", error);
      Swal.fire("Error", "Failed to save semester information", "error");
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "Semester record has been deleted.",
        toast: true,
        position: "top-end",
        timer: 3000,
        showConfirmButton: false,
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
        <div>
          ...
          <p className="text-muted small mb-0">
            Configure academic batches and semester durations
          </p>
        </div>
      </div>
      ...
      >
        ...
        <Nav.Item>
          <Nav.Link eventKey="form" className="px-4">
            {editingItem ? "Edit Semester" : "Add New"}
          </Nav.Link>
        </Nav.Item>
      </Nav>
